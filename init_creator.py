...
def FUNKTIONSTITEL():
	try:
		return render_template("FACH/ORDNER/HTMLNAME", aktFach="FACH", aktThema="THEMA", kartenURL="/FACHKARTENLINK", WGP_THEMEN=WGP_THEMEN, ITS_THEMEN=ITS_THEMEN, AWE_THEMEN=AWE_THEMEN)
	except Exception as e:
		print(str(e))
"""

vorlage_karten = """
@app.route('/FACHKARTENLINK/')
def FUNKTIONSTITEL():
	try:
		return render_template("FACH/ORDNER/HTMLNAME", aktFach="FACH", aktThema="THEMA", zurueckURL="/FACHLINKZURSEITE", WGP_THEMEN=WGP_THEMEN, ENG_THEMEN=ENG_THEMEN, ITS_THEMEN=ITS_THEMEN, AWE_THEMEN=AWE_THEMEN, Kartei=KARTEN_DICT["FACH"]["THEMA"])
	except Exception as e:
		print(str(e))
"""
...
for fach in THEMEN_DICT:
    ...
    for thema in THEMEN_DICT[fach]:
        ...
        print(vorlage_karten.replace("LINKZURSEITE", LINKZURSEITE).replace("KARTENLINK", KARTENLINK).replace("FUNKTIONSTITEL", KARTENFUNKTION).replace("FACH", FACH).replace("THEMA", THEMA).replace("HTMLNAME", HTMLNAME_KARTEN).replace("KARTENHTML", KARTENHTML).replace("ORDNER", ORDNER))
